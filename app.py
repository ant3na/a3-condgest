...
<style>
    ...
    @keyframes piscar {
      ...
      50% { opacity: 0.5; transform: scale(1.1); color: #f59e0b; }
...
def verificar_notificacoes_pendentes(sessao_db, perfil, condomino_id, user_id):
    """Verifica se existem notificações estruturais ativas para o utilizador atual, filtrando as já lidas na BD."""
    ...
    # Buscar os IDs lidos da Base de Dados para este utilizador específico
    ...
    # Alertas Globais: Assembleias nos próximos 5 dias ou em atraso por realizar
    ...
    # Alertas Exclusivos de Administração
    if perfil == "Admin":
...
def configurar_sidebar():
    ...
    if st.session_state.logado:
        ...
        # === CENTRAL DE NOTIFICAÇÕES DINÂMICA NA SIDEBAR ===
        tem_alertas = verificar_notificacoes_pendentes(session, st.session_state.perfil, st.session_state.condomino_id, st.session_state.user_id)
    ...
    with c2:
        ...
        with st.container(border=True):
                ...
                if submit:
                    if not user or not pwd:
                        ...
                        if utilizador_db and check_password_hash(utilizador_db.password_hash, pwd):
                            ...
                            registar_log(session, "LOGIN", f"Utilizador {utilizador_db.username} efetuou login com sucesso.")
                            session.commit()
                            st.rerun()
                        else: 
            ...
            <div style='text-align: center; margin-top: 15px;'>
                <p style='color: #94a3b8; font-size: 11px;'>© 2026 A3 Technologies | Versão 2.2</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        with st.form("form_pwd"):
            ...
            if st.form_submit_button("Atualizar Segurança"):
                if nova_pwd != conf_pwd: st.error("As passwords não coincidem!")
                    ...
                    registar_log(session, "ALTERAR_PASSWORD", f"O utilizador '{st.session_state.username}' alterou as suas credenciais de segurança.")
                    session.commit()
                    ...
                    if not user_existente:
                        ...
                        if st.button(":material/rocket_launch: Criar Utilizador", width="stretch"):
                            ...
                            try:
                                ...
                                registar_log(session, "CRIAR_UTILIZADOR", f"Criado utilizador '{username_sugerido}' para a fração '{cond_sel.fracao}'.")
                                session.commit()
                                ...
                                try:
                                    ...
                                    registar_log(session, "CRIAR_UTILIZADOR", f"Criado utilizador alternativo '{alt_username}' para a fração '{cond_sel.fracao}'.")
                                    session.commit()
                        ...
                        if c1.button("Repor Password para 'mudar123'", width="stretch"): 
                            ...
                            registar_log(session, "REPOR_PASSWORD", f"Reposta password padrão para utilizador '{user_existente.username}'.")
                            session.commit()
                            st.session_state.toast = ("Reposta!", "✅")
                            st.rerun()
                        if c2.button("Remover Acesso", width="stretch"): 
                            registar_log(session, "REMOVER_ACESSO", f"Removido permanentemente o utilizador '{user_existente.username}'.")
                ...
                with st.container(border=True):
                    ...
                    with st.form("form_perms"):
                        ...
                        if st.form_submit_button("Guardar Permissões Segmentadas", width="stretch"):
                            ...
                            registar_log(session, "ALTERAR_PERMISSOES", f"Alteradas permissões segmentadas do utilizador '{u_sel.username}'.")
                            session.commit()
                        ...
                        for _, row in df_imp.iterrows():
                            ...
                            if fracao and fracao != "nan":
                                ...
                                if not existe:
                                    ...
                                    novos += 1
                        if novos > 0:
                            registar_log(session, "IMPORTAR_CONDOMINOS", f"Importação em lote bem sucedida de {novos} moradores.")
                            session.commit()
            ...
            with st.form("f_cond"):
                ...
                if c1.form_submit_button("Guardar"):
                    if st.session_state.edit_type == "cond":
                        ...
                        registar_log(session, "EDITAR_CONDOMINO", f"Fração '{f}' modificada manualmente na base de dados.")
                        ...
                        registar_log(session, "CRIAR_CONDOMINO", f"Criada manualmente a nova fração de condomínio: '{f}'.")
            ...
            with st.container(border=True):
                ...
                if not st.session_state.modo_leitura:
                    ...
                    if c_del.button(":material/delete: Apagar Registo", width="stretch"):
                        registar_log(session, "APAGAR_CONDOMINO", f"Fração '{cond_obj.fracao}' removida permanentemente do sistema.")
                ...
                with col1:
                    if st.button(f":material/bolt: Gerar Quotas Apenas de {mes_str}", width="stretch"):
                        if len(condominos_sem_quota) > 0:
                            ...
                            registar_log(session, "GERAR_QUOTAS_MES", f"Quotas mensais relativas a '{mes_str}' geradas com sucesso.")
                        ...
                        for c in condominos:
                            for m in range(1, 13):
                                ...
                                if not session.query(Quota).filter_by(condomino_id=c.id, mes_ano=m_str).first():
                                    ...
                                    novas_quotas += 1
                        if novas_quotas > 0: 
                            registar_log(session, "GERAR_QUOTAS_ANO", f"Lançamento global anual de {novas_quotas} novas quotas para o ano civil '{ano_sel}'.")
            ...
            with st.expander(":material/forward_to_inbox: Enviar Avisos em Lote", expanded=False):
                ...
                if st.button("Disparar Avisos para Todos os Devedores", width="stretch", type="primary"):
                    ...
                    if emails_enviados > 0: 
                        registar_log(session, "AVISO_QUOTAS_LOTE", f"Disparados {emails_enviados} emails em massa de aviso de quotas em falta.")
                        session.commit()
                ...
                with st.container(border=True):
                    ...
                    if not st.session_state.modo_leitura:
                        if col_pagar.button(":material/done: Marcar como Paga", width="stretch"):
                            ...
                            registar_log(session, "PAGAR_QUOTA", f"Quota de {quota_obj.mes_ano} liquidada para a fração {quota_obj.condomino.fracao}.")
                                ...
                                if st.button("Confirmar Envio", key=f"mail_aviso_{quota_obj.id}", width="stretch"):
                                    if quota_obj.condomino.email:
                                        if enviar_email_real(quota_obj.condomino.email, f"Aviso de Pagamento - {quota_obj.mes_ano}", corpo_email): 
                                            registar_log(session, "AVISO_QUOTA_INDIVIDUAL", f"Notificação individual de quota enviada para a fração {quota_obj.condomino.fracao}.")
                                            session.commit()
                ...
                with st.form("f_orc"):
                    ...
                    if st.form_submit_button("Guardar Orçamento"):
                        if orc: 
                            ...
                            registar_log(session, "DEFINIR_ORCAMENTO", f"Orçamento atualizado para {v_orc} € no ano civil {ano_sel}.")
                        else: 
                            session.add(Orcamento(ano=ano_sel, valor_anual=v_orc))
                            registar_log(session, "DEFINIR_ORCAMENTO", f"Novo orçamento fixado em {v_orc} € para o ano {ano_sel}.")
                    ...
                    with st.form("f_mov"):
                        ...
                        if st.form_submit_button("Registar Lançamento"):
                            if not d.strip(): st.error("⚠️ O preenchimento da Descrição é obrigatório!")
                                ...
                                registar_log(session, f"LANÇAR_{t.upper()}", f"Lançamento manual de {t}: '{d}' no valor de {v} €.")
                    ...
                    if ficheiro_import_fin is not None:
                        if st.button("Processar Importação", width="stretch", type="primary"):
                            try:
                                ...
                                if novos_movs > 0:
                                    registar_log(session, "IMPORTAR_EXTRATO", f"Importação de {novos_movs} movimentos de extrato bancário efetuada com sucesso.")
                                    session.commit()
                ...
                if id_mov != "-": 
                    with st.container(border=True):
                        ...
                        if st.button(f":material/delete: Apagar {mov_obj.descricao}", width="stretch"): 
                            registar_log(session, "APAGAR_MOVIMENTO", f"Apagado o movimento financeiro '{mov_obj.descricao}' no valor de {mov_obj.valor} €.")
                    ...
                    if st.button("📧 Enviar Relatório por Email a Todos", width="stretch", type="primary"):
                        ...
                        for c in condominos_com_email:
                            ...
                            if enviar_email_real(c.email, f"Relatório de Contas Anual - {ano_sel}", corpo_email, anexo_bytes=pdf_bytes_anual, nome_anexo=f"Relatorio_Contas_{ano_sel}.pdf"):
                                emails_enviados += 1
                        if emails_enviados > 0: 
                            registar_log(session, "ENVIAR_RELATORIO_ANUAL", f"Relatório anual de contas de '{ano_sel}' enviado por email para {emails_enviados} moradores.")
                            session.commit()
                            ...
                            if st.button(":material/send: Enviar Confirmação Simples", width="stretch"):
                                if q.condomino.email:
                                    ...
                                    if enviar_email_real(q.condomino.email, f"Confirmação de Pagamento - {q.mes_ano}", corpo): 
                                        registar_log(session, "ENVIAR_CONFIRMACAO_RECIBO", f"Enviado email de confirmação simples de quota para '{q.condomino.fracao}'.")
                                        session.commit()
                                        st.toast("Enviado!", icon="✅")
                                else: st.error("Sem email registado.")
                            if REPORTLAB_INSTALLED:
                                if st.button("📧 Enviar Recibo com PDF Anexo", type="primary", width="stretch"):
                                    if q.condomino.email:
                                        ...
                                        if enviar_email_real(q.condomino.email, f"Recibo Oficial de Pagamento - {q.mes_ano}", corpo, anexo_bytes=pdf_bytes, nome_anexo=f"{nome_pdf}.pdf"): 
                                            registar_log(session, "ENVIAR_RECIBO_PDF", f"Recibo oficial em PDF anexo enviado por email para a fração '{q.condomino.fracao}'.")
                                            session.commit()
                                            st.toast("Enviado!", icon="🎉")
        ...
        with st.expander(":material/note_add: Arquivar Novo Documento", expanded=False):
            with st.form("form_upload"):
                ...
                if st.form_submit_button("Guardar Documento") and ficheiro is not None:
                    ...
                    registar_log(session, "ARQUIVAR_DOCUMENTO", f"Documento '{ficheiro.name}' inserido na categoria '{categoria}'.")
                    session.commit(); st.session_state.form_key += 1; st.rerun()
                ...
                if not st.session_state.modo_leitura:
                    ...
                    if pode_apagar:
                        if col_del.button("🗑️ Apagar", width="stretch"):
                            ...
                            registar_log(session, "APAGAR_DOCUMENTO", f"Ficheiro e registo de arquivo '{doc_obj.nome_ficheiro}' excluídos.")
                ...
                if c1.form_submit_button("Guardar Fornecedor"):
                    if not n.strip(): st.error("O nome é obrigatório.")
                        ...
                        if st.session_state.edit_type == "forn":
                            ...
                            registar_log(session, "EDITAR_FORNECEDOR", f"Atualizada a ficha técnica do fornecedor '{n}'.")
                            ...
                            registar_log(session, "CRIAR_FORNECEDOR", f"Fornecedor corporativo '{n}' adicionado ao catálogo.")
            ...
            with st.container(border=True):
                ...
                if st.session_state.perfil == "Admin" and not st.session_state.modo_leitura:
                    ...
                    if c_del.button(":material/delete: Apagar", width="stretch"):
                        registar_log(session, "APAGAR_FORNECEDOR", f"Fornecedor corporativo '{forn_obj.nome}' removido da base de dados.")
...
def pagina_ocorrencias():
    import time
    mes_sel, ano_sel, str_inicio, str_fim, mes_str = configurar_sidebar()
    st.header(":material/build: Gestão de Ocorrências e Operações")
                ...
                with st.form("f_oc"):
                    ...
                    if st.form_submit_button("Reportar Ocorrência"):
                        if not tit.strip() or not desc.strip(): st.error("⚠️ O preenchimento do Assunto e da Descrição é obrigatório!")
                            ...
                            registar_log(session, "REPORTAR_OCORRENCIA", f"Nova avaria/ocorrência submetida: '{tit}'.")
        ...
        if ocs:
            ...
            if evento_oc.selection.rows:
                ...
                with st.container(border=True):
                    ...
                    if not st.session_state.modo_leitura and st.session_state.perfil == "Admin":
                        if col_estado.button(":material/lock_open: Reabrir" if oc_obj.resolvida else ":material/check_circle: Resolver", key=f"btn_st_oc_{oc_obj.id}", width="stretch"):
                            oc_obj.resolvida = not oc_obj.resolvida
                            log_status = "REABRIR_OCORRENCIA" if not oc_obj.resolvida else "RESOLVER_OCORRENCIA"
                            registar_log(session, log_status, f"Alterado o estado técnico da ocorrência ID {oc_obj.id} para {'Pendente' if not oc_obj.resolvida else 'Resolvido'}.")
                            session.commit(); st.rerun()
                        if col_del.button(":material/delete: Apagar", key=f"btn_dl_oc_{oc_obj.id}", width="stretch"):
                            registar_log(session, "APAGAR_OCORRENCIA", f"Ocorrência técnica ID {oc_obj.id} excluída do histórico.")
    ...
    with tab_preventiva:
        st.write("### ⚙️ Plano de Manutenção Preventiva e Vistorias Periódicas")
        
        if st.session_state.perfil == "Admin" and not st.session_state.modo_leitura:
            with st.expander(":material/add_task: Programar Intervenção Preventiva", expanded=False):
                with st.form("f_manutencao"):
                    equip = st.text_input("Equipamento / Infraestrutura *", placeholder="Ex: Elevadores Bloco A, Extintores Gerais, Central de Bombas de Água", key=f"m_eq_{st.session_state.form_key}")
                    desc_m = st.text_input("Descrição do Procedimento Técnico *", placeholder="Ex: Inspeção Anual Obrigatória, Manutenção Trimestral do Motor", key=f"m_ds_{st.session_state.form_key}")
                    
                    c1, c2 = st.columns(2)
                    data_plan = c1.date_input("Data Planeada Técnico", value=hoje, key=f"m_dtp_{st.session_state.form_key}")
                    ...
                    fornecedores = session.query(Fornecedor).all()
                    opcoes_forn = {"Nenhum / Administração Geral": None}
                    for f in fornecedores: opcoes_forn[f.nome] = f.id
                    forn_sel = st.selectbox("Operador / Prestador Técnico Alocado", list(opcoes_forn.keys()), key=f"m_fr_{st.session_state.form_key}")
                    
                    obs_m = st.text_area("Observações / Notas Adicionais", key=f"m_ob_{st.session_state.form_key}")
                    
                    if st.form_submit_button("Agendar Intervenção"):
                        if not equip.strip() or not desc_m.strip():
                            st.error("⚠️ Os campos de Equipamento e Descrição são de preenchimento obrigatório!")
                        else:
                            nova_man = Manutencao(
                            ...
                            registar_log(session, "AGENDAR_MANUTENCAO", f"Agendada intervenção técnica para '{equip}' no dia {data_plan}.")
                            session.commit()
                            st.session_state.toast = ("Manutenção técnica programada com sucesso!", "✅")
                            st.session_state.form_key += 1; st.rerun()
        ...
        if manutencoes:
            with st.container(border=True):
                df_man = pd.DataFrame([{
                    ...
                    "Fornecedor Relacionado": session.get(Fornecedor, m.fornecedor_id).nome if m.fornecedor_id else "Administração"
            ...
            if evento_man.selection.rows:
                ...
                with st.container(border=True):
                    ...
                    forn_txt = session.get(Fornecedor, m_obj.fornecedor_id).nome if m_obj.fornecedor_id else "Administração"
                    col_m_info.info(f"**Ficha Preventiva:** {m_obj.descricao} em **{m_obj.equipamento}**")
                    ...
                    if not st.session_state.modo_leitura and st.session_state.perfil == "Admin":
                        if m_obj.estado != "Concluído":
                            if col_m_act.button("✅ Concluir Serviço", key=f"btn_dn_m_{m_obj.id}", width="stretch"):
                                m_obj.estado = "Concluído"
                                registar_log(session, "CONCLUIR_MANUTENCAO", f"Fechada a ordem técnica preventiva em '{m_obj.equipamento}'.")
                                session.commit(); st.rerun()
                        else:
                            if col_m_act.button("⏳ Reabrir Ordem", key=f"btn_ro_m_{m_obj.id}", width="stretch"):
                                m_obj.estado = "Pendente"
                                registar_log(session, "REABRIR_MANUTENCAO", f"Reaberta a ordem técnica preventiva de '{m_obj.equipamento}'.")
                                session.commit(); st.rerun()
                                
                        if col_m_del.button("🗑️ Cancelar Plano", key=f"btn_cc_m_{m_obj.id}", width="stretch"):
                            registar_log(session, "APAGAR_MANUTENCAO", f"Plano de manutenção preventiva de '{m_obj.equipamento}' excluído.")
                            session.delete(m_obj); session.commit(); st.rerun()
        else:
            st.info("Não existem manutenções preventivas programadas de momento.")
                ...
                with st.form("f_ass"):
                    ...
                    if st.form_submit_button("Agendar"):
                        if not tit.strip() or not assuntos.strip(): st.error("⚠️ Título e Ordem de Trabalhos obrigatórios.")
                            ...
                            registar_log(session, "AGENDAR_ASSEMBLEIA", f"Agendada assembleia geral: '{tit}' para a data {data_reuniao}.")
                    ...
                    if st.session_state.perfil == "Admin" and not st.session_state.modo_leitura:
                        if not r.realizada:
                            if col_act.button("Mark Realizada", key=f"real_{r.id}", width="stretch"): 
                                r.realizada = True
                                registar_log(session, "REALIZAR_ASSEMBLEIA", f"Assembleia ID {r.id} alterada para Realizada.")
                                session.commit(); st.rerun()
                        else:
                            if col_act.button("Reabrir", key=f"undo_{r.id}", width="stretch"): 
                                r.realizada = False
                                registar_log(session, "REABRIR_ASSEMBLEIA", f"Assembleia ID {r.id} reaberta da categoria concluída.")
                                session.commit(); st.rerun()
                        if col_act.button("🗑️ Eliminar", key=f"del_ass_{r.id}", width="stretch"): 
                            registar_log(session, "APAGAR_ASSEMBLEIA", f"Eliminado o registo de assembleia ID {r.id}.")
                        ...
                        if st.session_state.perfil == "Admin" and not st.session_state.modo_leitura:
                            with st.expander("📝 Redigir / Exportar Ata Digital", expanded=False):
                                ...
                                if st.button("💾 Gravar Ata", key=f"save_{r.id}", type="primary"):
                                    ...
                                    registar_log(session, "GRAVAR_ATA", f"Redigida/atualizada ata digital referente à assembleia ID {r.id}.")
                                ...
                                if r.texto_ata and REPORTLAB_INSTALLED:
                                    ...
                                    if c_a.button("🗂️ Arquivar no Portal", key=f"arq_{r.id}"):
                                        ...
                                        if not session.query(Documento).filter_by(nome_ficheiro=nome_ficheiro).first():
                                            ...
                                            registar_log(session, "ARQUIVAR_ATA", f"Ata digital da assembleia ID {r.id} arquivada automaticamente no repositório.")
                                    ...
                                    if c_m.button("📧 Enviar por Email", key=f"mail_{r.id}"):
                                        ...
                                        registar_log(session, "ENVIAR_ATA_EMAIL", f"Ata digital da assembleia ID {r.id} partilhada por email com {enviados} moradores.")
                                        session.commit()
                ...
                with st.form("f_sond"):
                    ...
                    if st.form_submit_button("Criar Votação"):
                        if not perg.strip() or not opcoes_str.strip(): st.error("Campos obrigatórios.")
                            ...
                            registar_log(session, "CRIAR_SONDAGEM", f"Publicada nova sondagem comunitária: '{perg}'.")
                            session.commit(); st.session_state.form_key += 1; st.rerun()
                ...
                with st.container(border=True):
                        ...
                        if ja_votou: st.success(f"Voto registado: {ja_votou.resposta}")
                            ...
                            with st.form(f"form_voto_{s.id}"):
                                ...
                                if st.form_submit_button("Votar"):
                                    ...
                                    registar_log(session, "VOTAR_SONDAGEM", f"Morador submeteu voto eletrónico na sondagem ID {s.id}.")
                                    session.commit(); st.rerun()
                    ...
                    if st.session_state.perfil == "Admin":
                        ...
                        if c_act1.button("Abrir/Fechar", key=f"alt_{s.id}"): 
                            s.ativa = not s.ativa
                            registar_log(session, "ALTERAR_ESTADO_SONDAGEM", f"Invertido o estado de ativação da sondagem ID {s.id} para {'Ativa' if s.ativa else 'Encerrada'}.")
                            session.commit(); st.rerun()
                        if c_act2.button("Apagar Votação", key=f"del_s_{s.id}"):
                            session.query(VotoSondagem).filter_by(sondagem_id=s.id).delete()
                            registar_log(session, "APAGAR_SONDAGEM", f"Sondagem ID {s.id} e os respetivos escrutínios removidos permanentemente.")
            ...
            with st.form("form_anuncio"):
                ...
                if st.form_submit_button("Publicar no Mural"):
                    if not titulo.strip() or not mensagem.strip():
                        ...
                        registar_log(session, "PUBLICAR_MURAL", f"Fixado novo anúncio social no mural: '{titulo}'.")
                ...
                if not st.session_state.modo_leitura:
                    ...
                    if pode_apagar:
                        if c_del.button("🗑️ Apagar", key=f"del_an_{a.id}", use_container_width=True):
                            registar_log(session, "APAGAR_MURAL", f"Anúncio social ID {a.id} apagado do feed comum.")
...
def pagina_notificacoes():
    ...
    st.write("Acompanhe aqui as pendências ativas e os alertas urgentes. As notificações desaparecem de forma dinâmica assim que resolver o assunto subjacente ou ao assinalar como lida.")
    
    alertas_encontrados = False
    
    # Buscar registos lidos pelo utilizador logado para não reexibir os ocultos
    registos_lidos = session.query(NotificacaoLida).filter_by(utilizador_id=st.session_state.user_id).all()
    lidos = [r.alerta_id for r in registos_lidos]

    # ---------------------------------------------------------
    # OPERAÇÕES ADMINISTRATIVAS (VISÍVEL APENAS PARA ADMIN)
    # ---------------------------------------------------------
    if st.session_state.perfil == "Admin":
        st.subheader("🛠️ Operações Pendentes da Administração")
        
        # 1. Ocorrências / Avarias sem resolução
        ...
        for o in ocs:
            ...
            if uid not in lidos:
                ...
                col_al.error(f"**Avaria por Resolver:** '{o.titulo}' (Reportado por {o.criado_por} em {o.data_criacao}).\n*Ação técnica pendente no menu Ocorrências.*", icon="🚨")
                with col_bt:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("❌ Ocultar Alerta", key=f"h_bt_{uid}", use_container_width=True):
                        session.add(NotificacaoLida(utilizador_id=st.session_state.user_id, alerta_id=uid))
                        session.commit(); st.rerun()
                        
        # 2. Vistorias e Manutenções Preventivas Pendentes
        ...
        for m in mans:
            ...
            if uid not in lidos:
                alertas_encontrados = True
                col_al, col_bt = st.columns([5, 1])
                col_al.warning(f"**Manutenção Preventiva:** '{m.descricao}' agendada para o equipamento **{m.equipamento}** em {m.data_planeada}.\n*Procedimento técnico requerido no menu Ocorrências.*", icon="⚙️")
                with col_bt:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("❌ Ocultar Alerta", key=f"h_bt_{uid}", use_container_width=True):
                        session.add(NotificacaoLida(utilizador_id=st.session_state.user_id, alerta_id=uid))
    ...
    # ---------------------------------------------------------
    # ALERTAS PESSOAIS E COMUNITÁRIOS (GERAL)
    # ---------------------------------------------------------
    st.subheader("👤 Os Meus Alertas Comunitários e Pessoais")
    
    # 3. Assembleias Próximas ou Agendas em Atraso por marcar Realizada
        ...
        if uid not in lidos:
            try:
                ...
                if 0 <= dias_restantes <= 5:
                    alertas_encontrados = True
                    col_al, col_bt = st.columns([5, 1])
                    col_al.info(f"**Assembleia Próxima:** A reunião geral '{a.titulo}' decorre em {dias_restantes} dia(s) ({a.data_agendada}).", icon="📅")
                    with col_bt:
                        st.markdown("<br>", unsafe_allow_html=True)
                        if st.button("❌ Ocultar Alerta", key=f"h_bt_{uid}", use_container_width=True):
                            session.add(NotificacaoLida(utilizador_id=st.session_state.user_id, alerta_id=uid))
                    ...
                    alertas_encontrados = True
                    col_al, col_bt = st.columns([5, 1])
                    col_al.error(f"**Convocatória Excedida:** A assembleia '{a.titulo}' tinha agendamento para {a.data_agendada} mas ainda não consta como efetuada no sistema.", icon="⚠️")
                    with col_bt:
                        st.markdown("<br>", unsafe_allow_html=True)
                        if st.button("❌ Ocultar Alerta", key=f"h_bt_{uid}", use_container_width=True):
                            session.add(NotificacaoLida(utilizador_id=st.session_state.user_id, alerta_id=uid))
    ...
    # 4. Tesouraria pessoal (Quotas em dívida)
    if st.session_state.condomino_id:
        ...
        for d in dividas:
            ...
            if uid not in lidos:
                alertas_encontrados = True
                col_al, col_bt = st.columns([5, 1])
                col_al.error(f"**Tesouraria Pessoal:** Encontra-se a pagamento a quota mensal de **{d.mes_ano}** no valor líquido de {d.valor:.2f} €.\n*Ação: Regularize por transferência bancária (IBAN disponível no Dashboard).* ", icon="💰")
                with col_bt:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("❌ Marcar Lida", key=f"h_bt_{uid}", use_container_width=True):
                        session.add(NotificacaoLida(utilizador_id=st.session_state.user_id, alerta_id=uid))
                        session.commit(); st.rerun()
                        
        # 5. Escrutínios e Sondagens ativas pendentes de participação
        ...
        for s in sond_ativas:
            ...
            if uid not in lidos:
                ...
                if not ja_votou:
                    alertas_encontrados = True
                    col_al, col_bt = st.columns([5, 1])
                    col_al.info(f"**Votação em Curso:** Sondagem pendente sobre '{s.pergunta}'. Exerça o seu direito de voto no painel comunitário.", icon="🗳️")
                    with col_bt:
                        st.markdown("<br>", unsafe_allow_html=True)
                        if st.button("❌ Marcar Lida", key=f"h_bt_{uid}", use_container_width=True):
                            session.add(NotificacaoLida(utilizador_id=st.session_state.user_id, alerta_id=uid))
    ...
    if not alertas_encontrados:
        st.success("🎉 Sensacional! Não possui alertas de pendências ativas neste momento. Tudo verificado e em dia!")

# ==========================================
# DECLARAÇÃO DA PÁGINA COMO OBJETO GLOBAL ESTÁVEL
# ==========================================
...
def gerar_snapshot_json():
    ...
    models_to_export = {
        ...
        "auditoria": Auditoria, "manutencoes": Manutencao, "notificacoes_lidas": NotificacaoLida
    }
...
def restaurar_snapshot_json(json_str):
    try:
        ...
        # 1. Limpar Tudo (Reset Suave Prévio Completo)
        session.query(NotificacaoLida).delete()
        ...
        registar_log(session, "RESTORE_BASEDADOS", "Injetado restauro estrutural completo de segurança por via de snapshot JSON.")
        session.commit()
        
        # 3. Sincronizar Sequências do PostgreSQL
        if engine.name == 'postgresql':
            from sqlalchemy import text
            tabelas_seq = ['votos_sondagem', 'sondagens', 'anuncios', 'assembleias', 'ocorrencias', 'fornecedores', 'documentos', 'movimentos', 'orcamentos', 'quotas', 'utilizadores', 'condominos', 'auditoria', 'manutencoes', 'notificacoes_lidas']
                ...
                with st.container(border=True):
                    with st.form("form_config"):
                        ...
                        if st.form_submit_button("Guardar Configurações"):
                            ...
                            registar_log(session, "ALTERAR_CONFIGURACOES", "Modificados os parâmetros base e metadados organizacionais do condomínio.")
                            session.commit()
                ...
                with st.container(border=True):
                    with st.form("form_avisos"):
                        ...
                        if st.form_submit_button("Atualizar Quadro de Avisos"):
                            ...
                            guardar_configs(config)
                            registar_log(session, "ALTERAR_QUADRO_AVISOS", f"Atualizado o quadro informativo global (Ativo: {aviso_ativo}).")
                            session.commit()
                            st.session_state.toast = ("Aviso updated!", "✅")
                            st.session_state.form_key += 1; st.rerun()
        ...
        with tab_seguranca:
            if not st.session_state.modo_leitura:
                with st.container(border=True):
                    ...
                    st.write("Processo de Exportação e Importação da Base de Dados num unico ficheiro estrutural (.json).")
                    
                    c_snap1, c_snap2 = st.columns(2)
                    with c_snap1:
                        st.write("**1. Backup Estrutural:**")
                        try:
                ...
                st.markdown("<br>", unsafe_allow_html=True)
                with st.container(border=True):
                    st.subheader("📊 Exportações Parciais (Excel/CSV)")
                ...
                with st.container(border=True):
                    ...
                    st.warning("Atenção: Esta operação limpa permanentemente todos os registos relacionais de todas as tabelas. A estrutura mantém-se disponível para novas importações.")
                    confirmar_reset = st.checkbox("Eu compreendo os riscos operacionais e pretendo formatar a base de dados.")
                    
                    if confirmar_reset:
                        if st.button("🔥 EXECUTAR RESET AGORA", type="primary"):
                            with st.spinner("A limpar dados do servidor..."):
                                try:   
                                    import time
                                    from sqlalchemy import text
                                    ...
                                    if engine.name == "postgresql":
                                        tabelas = ["votos_sondagem", "sondagens", "anuncios", "assembleias", "ocorrencias", "fornecedores", "documentos", "movimentos", "orcamentos", "quotas", "utilizadores", "condominos", "auditoria", "manutencoes", "notificacoes_lidas"]
                                        for tabela in tabelas:
                                            try: session.execute(text(f"ALTER SEQUENCE {tabela}_id_seq RESTART WITH 1;"))
                                            except Exception: session.rollback()
                                        session.commit()
                                    
                                    st.success("✔️ Formatação e limpeza de tabelas efetuada com sucesso.")
                                    time.sleep(1.5)
                                    st.session_state.logado = False
                                    st.session_state.username = None
                                    st.rerun()
                                    ...
                                    session.rollback()
                                    st.error(f"Erro técnico ao tentar limpar: {e}")

        with tab_auditoria:
            st.subheader("🕵️ Histórico de Auditoria do Portal")
            st.write("Consulta forense e transparente de todas as ações de escrita, alteração e eliminação efetuadas no ecossistema por qualquer utilizador.")
            ...
            if logs_db:
                ...
                st.info("Nenhuma atividade de escrita ou auditoria registada até à data.")
