...
# ==========================================
# BARRA LATERAL (LOGO, NOTIFICAÇÕES E FILTROS PREMIUM)
# ==========================================
hoje = date.today()
meses = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"]
opcoes_filtro = ["Todos (Ano Completo)"] + meses
...
def configurar_sidebar():
    ...
    if c_m.button("📅 Mês Atual", use_container_width=True):
        st.session_state.filtro_mes = meses[hoje.month - 1]
        st.session_state.filtro_ano = hoje.year
        st.rerun()
    if c_a.button("🗓️ Ano Atual", use_container_width=True):
        st.session_state.filtro_mes = "Todos (Ano Completo)"
        st.session_state.filtro_ano = hoje.year
        st.rerun()

    mes_sel = st.sidebar.selectbox("Mês de Trabalho", opcoes_filtro, key="filtro_mes")
    ano_sel = st.sidebar.selectbox("Ano de Trabalho", list(range(2020, hoje.year + 5)), key="filtro_ano")
    
    if mes_sel == "Todos (Ano Completo)":
        str_inicio = f"{ano_sel}-01-01"
        str_fim = f"{ano_sel+1}-01-01"
        mes_str = f"Ano {ano_sel}" 
    else:
...
def pagina_dashboard():
    ...
    label_periodo = f"Período referente a {mes_sel} {ano_sel}" if mes_sel != "Todos (Ano Completo)" else f"A visualizar Ano Completo de {ano_sel}"
    st.markdown(f"""
...
def pagina_quotas():
    ...
    if st.session_state.perfil == "Admin":
        ...
        if mes_sel == "Todos (Ano Completo)":
            pagas_query = session.query(Quota).filter(and_(Quota.paga == True, Quota.mes_ano.endswith(str(ano_sel))))
        else:
        ...
        if mes_sel == "Todos (Ano Completo)":
            pagas_query = session.query(Quota).filter(and_(Quota.paga == True, Quota.mes_ano.endswith(str(ano_sel)), Quota.condomino_id == st.session_state.condomino_id))
        else:
    ...
    if st.session_state.perfil == "Admin":
        with st.container(border=True):
            ...
            if mes_sel == "Todos (Ano Completo)":
                st.info(f"A visualizar o Ano Completo de {ano_sel}. Para gerar quotas de um mês específico, selecione-o no filtro lateral.")
                if not st.session_state.modo_leitura:
                    if st.button(f":material/calendar_month: Gerar Quotas para Todo o Ano de {ano_sel}", width="stretch", type="primary"):
                        novas_quotas = 0
                        for c in condominos:
                            for m in range(1, 13):
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    tab_dividas, tab_pagas = st.tabs([":material/error: Quotas em Dívida (A Cobrar)", f":material/check_circle: Quotas Recebidas"])
...
def pagina_financas():
    ...
    with tab_mes:
        ...
        with col_tit:
            st.subheader(f"Movimentos de {mes_sel} {ano_sel}" if mes_sel != "Todos (Ano Completo)" else f"Movimentos Totais de {ano_sel}")
            st.write(f"**Saldo Transitado Anterior:** {saldo_anterior:.2f} €")
            st.write(f"**(+) Total Quotas Recebidas no Período:** {q_atual:.2f} €")
        with col_btn:
            st.markdown("<br>", unsafe_allow_html=True)
            if REPORTLAB_INSTALLED:
